...
**B) A saída de calcularOrcamento() seria: 'Seu saldo é negativo de -1050.' e a de calcularOrcamento2() seria: 'Seu saldo é negativo de -100.'** //correto
...
**Compra 5 negada. Saldo insuficiente e limite de crédito excedido.** // correta
...
![Uma imagem](asset/imagem.png)
______

### Resposta:
```
Algoritmo "Boca de Urna"

inicioalgoritmo
    variavel idade <- recebe entrada ("Digite sua idade")
    se (idade < 16) entao
        imprime ("Você ainda não pode votar.")
    senao se (idade >= 16 && idade < 18) entao
        imprime ("Você pode votar, mas não é obrigado.")
    senao se (idade > 18) entao
        imprime ("Você é obrigado a votar, ou vai ter que pagar a taxa.")
    fimse
fimalgoritmo
```
...
### Resposta:
```
Algoritmo "aprendendo áreas de figuras geométricas"
...
inicioAlgoritmo
    classe FormaGeometrica
        ...
        Método calcularArea():
    fimclasseFormaGeometrica


    classe Retangulo
        Método construtor(cor, lado, altura)
            super(cor)

        sobreescrever calcularArea():
            lado*altura
...
```
```
Algoritmo "Velozes e furiosos"

inicioalgoritmo
    variavel tempoMaximo <- 30
    variavel tamanhoPista <- 6000
    variavel velocidadeMaxima <- 220
    variavel aceleracao <- 10
    variavel tempoVolta <- 0
    variavel velocidadeInicial<- 0

    enquanto (tempoVolta > tempoMaximo) faca
        acelerecao <- acelerecao + 1
        tempoEmMRUV <- (velocidadeMaxima - velocidadeInicial)/aceleracao
        tempoEmMRU <- {tamanhoPista - [(velocidadeInicial * tempoEmMRUV + aceleracao * tempoEmMRUV ^ 2)/2]}/velocidadeMaxima
